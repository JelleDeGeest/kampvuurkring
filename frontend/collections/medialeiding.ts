...
export const MediaLeiding: CollectionConfig = {
  slug: 'media-leiding',
  upload: {
    staticDir: path.resolve(process.cwd(), 'media-leiding'), // Separate directory for these uploads
  ...
  admin: {
    useAsTitle: 'filename',
    group: 'Media', // Optional: Group related collections in the admin UI
  },
  access: {
      ...
      async ({ doc, req, operation }) => {
        if (operation === 'create') {
          try {
            ...
            const uploadDir = path.resolve(process.cwd(), 'media-leiding');
